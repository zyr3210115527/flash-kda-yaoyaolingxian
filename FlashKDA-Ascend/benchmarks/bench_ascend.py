"""Time the Ascend forward pass, and report what limits it.

There is no GPU here running the CUDA version, so this cannot be a same-machine
comparison. It measures the Ascend kernel honestly and prints the workspace
footprint alongside, because the workspace is sized per (tile, head) with
[D,D] scratch slots and that is expected to bite well before the shapes the
CUDA benchmark uses (T=8192, H=64/96 on an H20).

Timing is wall clock around a synchronize: torch_npu's event API needs operator
kernels this CANN image does not ship.
"""
import math
import os
import sys
