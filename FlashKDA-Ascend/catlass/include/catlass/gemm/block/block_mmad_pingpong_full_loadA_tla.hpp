...
template <
    class ArchTag_, bool ENABLE_UNIT_FLAG_, bool ENABLE_SHUFFLE_K_, bool USE_HF32_MODE_, uint32_t L0C_STAGES_,
    bool ENABLE_L1_RESIDENT_, uint32_t L1A_STAGES_, uint32_t L1B_STAGES_, uint32_t L0A_STAGES_, uint32_t L0B_STAGES_,
...
struct BlockMmadTla<
    MmadFullLoadA<
        ArchTag_, ENABLE_UNIT_FLAG_, ENABLE_SHUFFLE_K_, USE_HF32_MODE_, L0C_STAGES_, ENABLE_L1_RESIDENT_, L1A_STAGES_,
...
public:
    // Type Aliases
    using DispatchPolicy = MmadFullLoadA<
        ArchTag_, ENABLE_UNIT_FLAG_, ENABLE_SHUFFLE_K_, USE_HF32_MODE_, L0C_STAGES_, ENABLE_L1_RESIDENT_, L1A_STAGES_,
        L1B_STAGES_, L0A_STAGES_, L0B_STAGES_>;
        ...
        TensorBias const& tensorBias = {})
    {
        ...
        uint32_t kL1Actual = (firstTileIdx < kTileCount - 1) ? L1_TILE_K : (kBlockActual - firstTileIdx * L1_TILE_K);
        ...
        auto tensorTileB =
            GetTile(tensorB, tla::MakeCoord(firstTileIdx * L1_TILE_K, 0), tla::MakeShape(kL1Actual, nBlockActual));
        if constexpr (ENABLE_L1_RESIDENT) {
            ...
            for (int mL0Idx = 0; mL0Idx < mL0Loop; mL0Idx++) {
                ...
                for (int kL0Idx = 0; kL0Idx < kL0Loop; kL0Idx++) {
                    ...
                    auto tensorTileL1A = GetTileA(
                        tensorL1A, mL0Idx * L0_TILE_M, kL0Idx * L0_TILE_K + shuffleKIdx * L1_TILE_K, mL0Actual,
                        kL0Actual);
                    ...
                    for (int nL0Idx = 0; nL0Idx < nL0Loop; nL0Idx++) {
                        ...
                        if constexpr (ENABLE_UNIT_FLAG) {
                            if ((kL1Idx == kTileCount - 1) && (mL0Idx == mL0Loop - 1) && (kL0Idx == kL0Loop - 1) &&
                                (nL0Idx == nL0Loop - 1)) {
                                unitFlag = 0b11;
                            } else {
                                unitFlag = 0b10;
