...
public:
    // Type Aliases
    using DispatchPolicy = MmadDequant<
        ArchTag_, ENABLE_UNIT_FLAG_, USE_HF32_MODE_, L0C_STAGES_, ENABLE_L1_RESIDENT_, L1A_STAGES_, L1B_STAGES_,
        L0A_STAGES_, L0B_STAGES_>;
    ...
    static_assert(
        (!HAS_BIAS && !HAS_QUANT_TENSOR && (L1A_STAGES + L1B_STAGES) <= 8) ||
    ...
    template <
        class TensorA, class TensorB, class TensorC, class TensorBias = EmptyClass, class TensorQuant = EmptyClass>
        ...
        TensorBias const& tensorBias = {}, TensorQuant const& tensorQuant = {}, float scaleParam = 1.0)
    {
        ...
        if constexpr (HAS_BIAS) {
            ...
            static_assert(L1AB_TILE_SIZE + L1BIAS_SIZE <= ArchTag::L1_SIZE, "L1TileShape exceeding the L1 space!");
        }
        if constexpr (HAS_QUANT_TENSOR) {
            ...
            if constexpr (HAS_BIAS) {
                static constexpr uint32_t L1BIAS_SIZE = L1_TILE_N * sizeof(ElementBias);
                static_assert(
                    L1AB_TILE_SIZE + L1BIAS_SIZE + L1QUANT_SIZE <= ArchTag::L1_SIZE,
                    "L1TileShape exceeding the L1 space!");
            } else {
                static_assert(L1AB_TILE_SIZE + L1QUANT_SIZE <= ArchTag::L1_SIZE, "L1TileShape exceeding the L1 space!");
            }
        }
