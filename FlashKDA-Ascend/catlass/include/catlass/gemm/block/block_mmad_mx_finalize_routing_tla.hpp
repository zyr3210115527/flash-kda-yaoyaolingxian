...
#ifndef CATLASS_GEMM_BLOCK_BLOCK_MMAD_MX_FINALIZE_ROUTING_TLA_HPP
#define CATLASS_GEMM_BLOCK_BLOCK_MMAD_MX_FINALIZE_ROUTING_TLA_HPP
...
struct BlockMmadMxFinalizeRoutingTla {
    static_assert(
        DEPENDENT_FALSE<DispatchPolicy>, "BlockMmadMxFinalizeRoutingTla is not implemented for this DispatchPolicy");
};

template <
    class ArchTag_, bool ENABLE_UNIT_FLAG_, uint32_t L1_SCALE_FACTOR_K_, uint32_t L0C_STAGES_, bool ENABLE_L1_RESIDENT_,
...
public:
    ...
    /// Construct
    CATLASS_DEVICE
    BlockMmadMxFinalizeRoutingTla(Arch::Resource<ArchTag>& resource, uint32_t l1BufAddrStart = 0)
    {
    ...
    /// Perform a block-scoped matrix multiply-accumulate with cross-core sync.
    /// needWait: whether to wait for AIV→AIC flag before mmad (skipped on first tile).
    /// waitFlagId / setFlagId: cross-core flag id to wait/set around mmad.
    template <
        uint16_t SYNC_MODE, class TensorA, class TensorB, class TensorC, class TensorMxScaleA = EmptyClass,
        class TensorMxScaleB = EmptyClass, class TensorBias = EmptyClass>
    CATLASS_DEVICE void operator()(
        TensorA& tensorA, TensorB& tensorB, TensorC& tensorC, GemmCoord const& actualShape, bool needWait,
        uint16_t waitFlagId, uint16_t setFlagId, TensorMxScaleA const& tensorMxScaleA = {},
        TensorMxScaleB const& tensorMxScaleB = {}, TensorBias const& tensorBias = {})
    {
        ...
        for (uint32_t kL1Idx = 0; kL1Idx < kL1Loop; kL1Idx++) {
            ...
            if (kL1Idx < kL1Loop - 1) {
                ...
                if (kL1IdxNext % L1_SCALE_FACTOR_K == 0) {
                    ...
                    auto tensorL1MxScaleA_pre = tla::MakeTensor(
                        l1MxScaleATensorList[l1MxScaleAListIdNext], L1SCALEA_LAYOUT, Arch::PositionL1{});
                    auto tensorTileMxScaleA = GetTile(
                        tensorMxScaleA, tla::MakeCoord(0, kL1ScaleIdxNext * l1ScaleTileK / MX_SCALE_GROUP_NUM),
                ...
                if (kL1IdxNext % L1_SCALE_FACTOR_K == 0) {
                    ...
                    uint32_t kL1MxScaleActualNext = (kL1ScaleIdxNext < kL1ScaleLoop - 1) ?
                                                        l1ScaleTileK :
                                                        (kBlockActual - kL1ScaleIdxNext * l1ScaleTileK);
                    auto tensorL1MxScaleB_pre = tla::MakeTensor(
                        l1MxScaleBTensorList[l1MxScaleBListIdNext], L1SCALEB_LAYOUT, Arch::PositionL1{});
                    auto tensorTileMxScaleB = GetTile(
                        tensorMxScaleB, tla::MakeCoord(kL1ScaleIdxNext * l1ScaleTileK / MX_SCALE_GROUP_NUM, 0),
        ...
        // Notify AIV that this tile's FIX is done (originally PIPE_FIX SetFlag at kernel side).
        AscendC::CrossCoreSetFlag<SYNC_MODE, PIPE_FIX>(setFlagId);
    }
...
#endif // CATLASS_GEMM_BLOCK_BLOCK_MMAD_MX_FINALIZE_ROUTING_TLA_HPP
