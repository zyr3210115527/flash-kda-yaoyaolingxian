...
struct BlockMmadTla<
    MmadFAITailQK<ArchTag_, PAGED_CACHE_FLAG_, ENABLE_UNIT_FLAG_>, L1TileShape_, L0TileShape_, ElementA_, ElementB_,
...
public:
    ...
    CATLASS_DEVICE void operator()(
        TensorA& tensorA, TensorB& tensorB, AscendC::GlobalTensor<ElementC> gC,
        AscendC::GlobalTensor<int32_t> gBlockTable, GemmCoord actualOriShape, uint32_t& nIdx, uint32_t& nLoop,
        uint32_t& blockSize, uint32_t kvSeqlen, uint32_t strideKV, uint32_t maskTailS, bool preloadFlag)
    {
        ...
        for (uint32_t blockStackIdx = 0; (blockStackIdx < UNIT_BLOCK_STACK_NUM) && ((nIdx + blockStackIdx) < nLoop);
            ...
            for (uint32_t kIdx = 0; kIdx < kLoop; kIdx++) {
                ...
                computeQK(
                    tensorA, tensorB, tensorCOffset, gBOffset, gBNextOffset, actualShape, actualNextShape,
                    blockStackIdx, nkBlockNextIdx, nkBlockLoop, strideKV, firstItr, endItr, initMmad, firstQtr, endQItr,
                    preloadFlag);
                ++nkBlockNextIdx;
                preloadFlag = false;
            }
    ...
    CATLASS_DEVICE void computeQK(
        ...
        uint32_t strideKV, bool firstItr, bool endItr, bool initMmad, bool firstQItr, bool endQItr, bool preloadFlag)
    {
