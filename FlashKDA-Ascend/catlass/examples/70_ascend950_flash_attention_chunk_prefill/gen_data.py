...
class TestFlashAttentionInfer:
    ...
    def softmax1(self, qk_result, is_first, gm, is_kvs_last_loop, data_type=np.float16):
        sim = qk_result
    ...
    def ref_flash_attention(
        ...
        if kv_start < context_len:
            ...
            if kv_start == 0:
                gm = None
            p_result, row_sum, dm, gm = self.softmax1(
                qk_result, kv_start == 0, gm, is_kvs_last_loop
            )
            p_result = p_result.astype(data_type)
            if kv_start == 0:
                gm_high = None
            p_result_high, row_sum_high, dm_high, gm_high = self.softmax1(
                qk_result_high, kv_start == 0, gm_high, is_kvs_last_loop
            )
        ...
        for kv_start in range(kv_start, context_len, context_size):
            ...
            p_result, row_sum, dm, gm = self.softmax1(
                qk_result, False, gm, is_kvs_last_loop
            )
            p_result = p_result.astype(data_type)
            p_result_high, row_sum_high, dm_high, gm_high = self.softmax1(
                qk_result_high, False, gm_high, is_kvs_last_loop
            )
            lo = self.group_matmul(
                p_result.shape[0], sub_value.shape[0], p_result, sub_value
            )
    ...
    def ref_masked_attention(self, query, key, value, scale: float, mask):
        query = np.transpose(query, (1, 0, 2))
        key = np.transpose(key, (1, 2, 0))
    ...
    def calc_data(self, gen_data_params: GenDataParams):
        ...
        for i in range(batch_size):
            ...
            tri = np.triu(tri, 1)
            tri *= pre_mask_factor
            mask[pre_qseqlen : (pre_qseqlen + qseqlen), kseqlen - qseqlen : kseqlen] = (
                tri
            )
            pre_qseqlen += qseqlen
        ...
        if gen_data_params.cache_layout == "nd":
            ...
            key_cache_nz = key_cache.reshape(
                gen_data_params.num_blocks,
                gen_data_params.kv_heads,
                gen_data_params.block_size,
                head_size_qk // 16,
            ...
            value_cache_nz = value_cache.reshape(
                gen_data_params.num_blocks,
                gen_data_params.kv_heads,
                gen_data_params.block_size,
                head_size_vo // 16,
...
if __name__ == "__main__":
    ...
    if num_blocks < required_min_blocks:
        print(
            f"[ERROR] numBlocks ({num_blocks}) is less than minimum required "
