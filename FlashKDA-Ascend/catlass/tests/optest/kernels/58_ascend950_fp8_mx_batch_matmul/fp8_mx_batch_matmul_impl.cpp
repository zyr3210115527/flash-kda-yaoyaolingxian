...
extern "C" void run(uint32_t blockNum, aclrtStream stream, const CatlassKernel::MatmulParams* params)
{
    ...
    uint32_t taskNum = batchCount * CeilDiv(m, tla::get<0>(L1TileShape{})) *
    ...
    if (m > n) {
        ...
        using MatmulKernel = Catlass::Gemm::Kernel::MxBatchedMatmulTla<BlockMmad, BlockEpilogue, BlockScheduler>;
        typename MatmulKernel::Arguments arguments{
            batchCount, problemShape, deviceA, layoutA, deviceB, layoutB, deviceMxScaleA, layoutMxScaleA,
            deviceMxScaleB, layoutMxScaleB, deviceC, layoutC};
