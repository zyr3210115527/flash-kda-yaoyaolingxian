...
public:
    ...
    CATLASS_DEVICE void operator()(Params const &params)
    {
        ...
        uint32_t processNum = batch * curQheadSplitNum * maxKvSplitCoreNum;
        ...
        for (uint32_t process = coreIdx; process < processNum; process += uint32_t(coreNum)) {
            ...
            uint32_t curBatch = realProcess / (curQheadSplitNum * maxKvSplitCoreNum);
            ...
            uint32_t qHeadSplitIdx = (process % (curQheadSplitNum * maxKvSplitCoreNum)) / maxKvSplitCoreNum;
            ...
            uint32_t curNIdx = process % maxKvSplitCoreNum;
            ...
            for (uint32_t nIdx = 1; nIdx < nLoop + 1; nIdx++, pingpongIdx++) {
                ...
                if (nIdx == nLoop) {
                    ...
                    for (uint32_t nextProcess = uint32_t(coreNum) + process; nextProcess < processNum;
                        ...
                        uint32_t nextBatch = nextRealProcess / (curQheadSplitNum * maxKvSplitCoreNum);
                        ...
                        uint32_t nextQHeadSplitSizeActual = (nextQHeadSplitIdx == (curQheadSplitNum - 1))
                                                                ...
                                                                : curQheadSplitSize;
                        uint32_t nextCurNIdx = nextProcess % maxKvSplitCoreNum;
