...
/**
 * @brief example 74_ascend950_weight_quant_a8w4_grouped_mx_matmul:
 *        Resolve and launch the JIT grouped A8W4 MX matmul implementation.
 */
extern "C" void Ascend950A8W4GroupedMxMatmul(
    const uint32_t blockNum, aclrtStream stream, const TParams& tParams, const GroupedMatmulParams& params)
{
    auto* entry = JitCompiler::instance().getKernel(
        "weight_quant_a8w4_grouped_mx_matmul_impl.cpp",
        JitMacroGenerator<TParams>::generate("ascend950_a8w4_grouped_mx_matmul", tParams),
        JitKernelType::MIX);
    if (entry) {
