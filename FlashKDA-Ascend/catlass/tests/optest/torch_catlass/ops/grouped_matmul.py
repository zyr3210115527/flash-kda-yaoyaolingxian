...
def grouped_matmul(
    ...
    """Run CATLASS grouped matmul (general K-slice) on NPU tensors.

    Source: example 08_grouped_matmul.

    A and B are single 2-D matrices shared across all groups. A is ``(M, K)``
    (or ``(K, M)`` if transA). B is ``(K, N)`` (or ``(N, K)`` if transB).
    groupList is a 1-D int64 prefix-sum array of length G on NPU.
    ...
    Args:
        ...
        transA: Whether ``mat1`` is transposed (stored as ``(K, M)``).
        transB: Whether ``mat2`` is transposed (stored as ``(N, K)``).
    ...
    return torch.ops.catlass.grouped_matmul(
