...
def ascend950_batched_matmul(
    ...
    """Run CATLASS Ascend950 batched matmul TLA on NPU tensors.

    Source: example 67_ascend950_batched_matmul.

    Input tensors are 3D with shape ``(batch, M, K)`` and ``(batch, K, N)``
    (or 2D ``(M, K)`` / ``(K, N)`` for batch=1).

    Args:
        mat1: Left input matrix. Shape ``(B, M, K)`` unless ``transA`` is true.
        mat2: Right input matrix. Shape ``(B, K, N)`` unless ``transB`` is true.
    ...
    return torch.ops.catlass.ascend950_batched_matmul(
