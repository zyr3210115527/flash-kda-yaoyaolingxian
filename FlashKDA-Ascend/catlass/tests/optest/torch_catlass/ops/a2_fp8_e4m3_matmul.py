import torch
from torch import Tensor


def a2_fp8_e4m3_matmul(
    mat1: Tensor,
    mat2: Tensor,
    outDType: str | torch.dtype = torch.float16,
    transA: bool = False,
    transB: bool = False,
    formatA: bool = False,
    formatB: bool = False,
) -> Tensor:
    """Run CATLASS FP8 E4M3 matmul on NPU tensors.

    Source: example 29_a2_fp8_e4m3_matmul.

    Computes ``C = fp16(A) @ fp16(B)`` where both A and B are FP8 (int8
    stored) matrices that are dequantized to fp16 via prologue before
    the matmul.

    Args:
        mat1: Left input matrix (int8 representing FP8). Shape ``(M, K)``.
        mat2: Right input matrix (int8 representing FP8). Shape ``(K, N)``.
        outDType: Output dtype. Default ``float16``.
        transA: Whether to read ``mat1`` as transposed.
        transB: Whether to read ``mat2`` as transposed.
        formatA: Whether ``mat1`` is in CATLASS NZ block format.
        formatB: Whether ``mat2`` is in CATLASS NZ block format.

    Returns:
        Output tensor with shape ``(M, N)`` on the active NPU device.
    """
    if isinstance(outDType, str):
        dtype_lower = outDType.lower()
        outDType = getattr(torch, dtype_lower, None)
    if outDType is None:
        raise ValueError(f"{outDType} is not a data type of torch")
    return torch.ops.catlass.a2_fp8_e4m3_matmul(
        mat1, mat2, outDType, transA, transB, formatA, formatB
    )
