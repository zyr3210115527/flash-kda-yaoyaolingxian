...
def matmul_add(
    ...
    """Run CATLASS matmul with element-wise add on NPU tensors.

    Computes ``D = A @ B + X``, where X is the addend.

    Source: example 03_matmul_add.

    Args:
        mat1: Left input matrix. Shape ``(M, K)`` unless ``transA`` is true.
        mat2: Right input matrix. Shape ``(K, N)`` unless ``transB`` is true.
        x: Addend tensor. Shape ``(M, N)``.
    ...
    return torch.ops.catlass.matmul_add(
        mat1, mat2, x, outDType, transA, transB, formatA, formatB
    )
