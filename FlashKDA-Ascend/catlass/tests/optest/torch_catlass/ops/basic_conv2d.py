...
def basic_conv2d(
    ...
    """Run CATLASS basic 2D convolution on NPU tensors.

    Source: example 33_basic_conv2d.

    Args:
        fmap: Input feature map tensor ``(N, C, H, W)``.
        filter: Filter tensor ``(OC, IC, KH, KW)``.
    ...
    """
    return torch.ops.catlass.basic_conv2d(
        fmap,
        filter,
        stride,
        padding,
