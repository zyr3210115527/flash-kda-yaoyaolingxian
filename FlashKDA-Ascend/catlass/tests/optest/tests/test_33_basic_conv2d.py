...
def test_basic_conv2d():
    ...
    result = torch_catlass.basic_conv2d(
        fmap_npu,
        filter_npu,
        stride=stride,
        padding=padding,
    ...
    result_cpu_flat = result.cpu()
    result_nchw = nc1hwc0_to_nchw(result_cpu_flat, N, OC, Ho, Wo)
...
@only_on_2201
def test_basic_conv2d_exceeds_cache():
    torch.manual_seed(2)
    N, C, H, W = 1, 16, 8, 8
    OC, KH, KW = 128, 5, 5
    stride = [1, 1]
