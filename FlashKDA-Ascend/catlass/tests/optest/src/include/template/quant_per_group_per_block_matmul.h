...
#ifndef OPTEST_QUANT_PER_GROUP_PER_BLOCK_MATMUL_H
#define OPTEST_QUANT_PER_GROUP_PER_BLOCK_MATMUL_H
...
struct QuantPerGroupPerBlockMatmulLike {
        ...
        CatlassKernel::MatmulParams& params)
    {
        ...
        tParams.element["X1SCALE"] = TorchDtypeToAclDtype(x1Scale.scalar_type());
        tParams.element["X2SCALE"] = TorchDtypeToAclDtype(x2Scale.scalar_type());
        tParams.element["Y"] = TorchDtypeToAclDtype(outDType);
        ...
        params.inputAddr[2] = static_cast<uint8_t*>(const_cast<void*>(x1Scale.storage().data()));
        params.inputAddr[3] = static_cast<uint8_t*>(const_cast<void*>(x2Scale.storage().data()));
        ...
        const CatlassKernel::TParams& tParams, CatlassKernel::MatmulParams& params)
    {
        OutputType output = GetOutputTensor(
            {params.m, params.n}, AclDtypeToTorchDtype(tParams.elem("Y")));
        ...
        bool formatA, bool formatB)
    {
        CatlassKernel::TParams tParams;
        CatlassKernel::MatmulParams params;
        GetKernelInfo(mat1, mat2, x1Scale, x2Scale, outDType, transA, transB, formatA, formatB, tParams, params);
