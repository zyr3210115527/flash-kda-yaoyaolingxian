...
using BatchedKernelFn = void (*)(const uint32_t, aclrtStream, const CatlassKernel::TParams&, const CatlassKernel::MatmulParams&);

template <BatchedKernelFn KernelFunc>
struct BatchedMatmulLike {
        ...
        CatlassKernel::MatmulParams& params)
    {
        ...
        if (transA) {
            m = mat1.size(dimA);  k1 = mat1.size(dimA - 1);
        } else {
            m = mat1.size(dimA - 1);  k1 = mat1.size(dimA);
        }
        if (transB) {
            k2 = mat2.size(dimB); n = mat2.size(dimB - 1);
        } else {
            k2 = mat2.size(dimB - 1); n = mat2.size(dimB);
        }
