...
#ifndef OPTEST_TRMM_H
#define OPTEST_TRMM_H
...
using TrmmKernelFn =
    void (*)(const uint32_t, aclrtStream, const CatlassKernel::TParams&, const CatlassKernel::TrmmParams&);

template <TrmmKernelFn KernelFunc>
struct TrmmLike {
    ...
    static OutputType Run(
        ...
        int64_t trans, int64_t diag, double alpha)
    {
        TORCH_CHECK(mat1.dim() == 2 && mat2.dim() == 2, "trmm expects two 2-D tensors");
        TORCH_CHECK(side == 0 || side == 1, "trmm side must be 0 (left) or 1 (right)");
        TORCH_CHECK(uplo == 0 || uplo == 1, "trmm uplo must be 0 (lower) or 1 (upper)");
        TORCH_CHECK(trans == 0 || trans == 1, "trmm trans must be 0 (no transpose) or 1 (transpose)");
        TORCH_CHECK(diag == 0, "trmm currently supports only diag=0 (non-unit diagonal)");
        TORCH_CHECK(
            mat1.scalar_type() == at::kFloat && mat2.scalar_type() == at::kFloat,
            "trmm currently supports float32 inputs");
        TORCH_CHECK(outDType == at::kFloat, "trmm currently supports float32 output");

        CatlassKernel::TParams tParams;
        CatlassKernel::TrmmParams params;
        ...
        tParams.transpose["A"] = (side == 0 && trans == 1);
        tParams.transpose["B"] = (side == 1 && trans == 1);
        tParams.useNz["A"] = false;
        tParams.useNz["B"] = false;
        tParams.useNz["C"] = false;

        if (side == 0) {
            TORCH_CHECK(mat1.size(0) == mat1.size(1), "left trmm expects mat1 to be square");
            TORCH_CHECK(mat1.size(1) == mat2.size(0), "left trmm shape mismatch");
            ...
            TORCH_CHECK(mat2.size(0) == mat2.size(1), "right trmm expects mat2 to be square");
            TORCH_CHECK(mat1.size(1) == mat2.size(0), "right trmm shape mismatch");
            params.m = static_cast<uint32_t>(mat1.size(0));
        ...
        params.trans = static_cast<uint32_t>(trans);
        params.diag = static_cast<uint32_t>(diag);
